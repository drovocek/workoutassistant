...
class WorkoutWorkoutSchemaToPGobjectWritingConverterTest {

    private final ObjectMapper jsonMapper = CustomJsonObjectMapper.instance();
    private final WorkoutSchemaToPGobjectWritingConverter writingConverter = new WorkoutSchemaToPGobjectWritingConverter(jsonMapper);
    ...
    void convert(WorkoutSchema workoutSchema) {
        PGobject actual = this.writingConverter.convert(workoutSchema);
        PGobject expected = PGobject(workoutSchema);
        Assertions.assertEquals(expected, actual);
    }

    public PGobject PGobject(WorkoutSchema forWriting) {
        return wrapper(
                () -> {
